...
- **Main Features (Phase1 Mock)**:
  ...
  - 追加ページトッピング反映（エディタで追加ページを作成・編集し、公開URL `/sites/[subdomain]/[slug]` へ反映）
...
6. `/editor/site-001` 下部の「トッピング（このHP）」でHP単位トッピングON/OFF確認（予約フォームON時は編集パネルとプレビューに予約機能が表示）
7. `追加ページ`トッピングをONにし、`追加ページ設定`でページを作成後、`/sites/[subdomain]/[slug]` で表示確認
8. 公開ページ `/sites/[subdomain]` のお問い合わせフォームから送信し、`/dashboard` の「最新のお問い合わせ」で受信確認
9. `/inbox` でお問い合わせを未対応/対応中/対応済みに更新
10. `/publish` でサブドメイン重複チェックを試す
11. `/billing` で料金表示と休眠プラン導線を確認
