...
- **Main Features (Phase1 Mock)**:
  ...
  - HPエディタ（自動保存デバウンス、保存状態表示、セクションON/OFF・並び替え、OFFセクションの編集項目自動非表示、改行入力対応、コントラスト警告、各セクション本文編集、料金編集、画像アップロード[5MB以内]、画像削除、画像情報表示[形式/サイズ/解像度/比率]、推奨比率警告）
  - トッピング管理（HPごと / エディタ画面内でON・OFF可能）
  - トッピング連動プレビュー（予約フォーム・SNS・ブログ・SEO・GAの反映）
  - 追加ページトッピング反映（エディタで追加ページを作成・編集し、公開URL `/sites/[subdomain]/[slug]` へ反映。公開サイト上部ナビにも自動表示）
  - お問い合わせフォーム実装（公開ページから名前/メール/本文を送信し、ダッシュボードと問い合わせ管理画面で受信・未対応/対応中/対応済み管理）
  - 公開サイトデザイン強化（テンプレート別テーマ色、余白設計、ヒーロー/サービス/FAQ/予約/問い合わせ導線の実戦レイアウト）
...
8. `追加ページ`トッピングをONにし、`追加ページ設定`でページを作成後、`/sites/[subdomain]/[slug]` と公開ページ上部ナビ表示を確認
9. 公開ページ `/sites/[subdomain]` のお問い合わせフォームから送信し、`/dashboard` の「最新のお問い合わせ」で受信確認
10. `/inbox` でお問い合わせを未対応/対応中/対応済みに更新
11. `/publish` でサブドメイン重複チェックを試す
12. `/billing` で料金表示と休眠プラン導線を確認
...
- **Last Updated**: 2026-04-18（デザイン改善版）
